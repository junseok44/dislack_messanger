...
const Channel = () => {
  ...
  return (
    <div className="flex h-full max-h-screen flex-grow">
      <ChannelSideBar
        channels={channels}
        ...
        userId={user?.id}
      />
      <div className="flex h-full flex-grow flex-col bg-background-dark">
        <div className="flex h-12 flex-shrink-0 border-b-2 border-primary-dark p-2">
          <div className="flex items-center gap-1">
            <Hash className="text-white" size={20} />
            {currentChannel ? (
              <Typography size="large">{currentChannel.name} 채널</Typography>
            ) : (
              <h1 className="p-4 text-2xl text-white">Channel not found</h1>
            )}
          </div>
        </div>
